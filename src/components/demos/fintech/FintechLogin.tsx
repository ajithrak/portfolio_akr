...
interface FintechLoginProps {
  onSuccess?: () => void;
}

export const FintechLogin: React.FC<FintechLoginProps> = ({ onSuccess }) => {
  const [step, setStep] = useState<'credentials' | '2fa'>('credentials');
  const [email, setEmail] = useState('alex.morgan@nexusbank.com');
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if (value && index < 5) {
      const nextInput = document.getElementById(`fintech-otp-${index + 1}`);
      nextInput?.focus();
    }
  };
    ...
    <div className="relative min-h-[720px] bg-slate-950 text-slate-100 flex flex-col justify-center items-center px-4 py-8 antialiased selection:bg-emerald-500 selection:text-white overflow-hidden">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-white">Nexus Bank</h1>
        ...
        <div className="bg-slate-900/80 border border-slate-800 backdrop-blur-xl rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/60">
          ...
          {step === 'credentials' ? (
            <form onSubmit={handleCredentialsSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1.5" htmlFor="fintech-email">
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <label className="text-xs font-medium text-slate-300" htmlFor="fintech-password">
                    Password
                  </label>
